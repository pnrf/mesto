...
* после перерыва вернулся к выполнению данной проектной работы;

**18.05.2022** (ПР 8)

* разбираюсь, почему код не работает. Устраняю ошибки;

**19.05.2022** (ПР 8)

* завершил работу над ошибками. Весь функционал реализован. Посмотрим, что скажет код-ревьюер;
* наcтроил сборку проекта Вебпаком;
* отправил на проверку код-ревьюеру. Жду замечаний.

**20.05.2022** (ПР 8)

* получил ответ от код-ревьюера: 19 критических замечаний и 2 рекомендации. Работаю над исправлениями.

**21.05.2022** (ПР 8)

* устранил замечания код-ревьюера;
* отправил на новую проверку.

**23.05.2022** (ПР 8)

* получил ответ от код-ревьюера - в целом все ок, осталось исправить пару шероховатостей;
* исправил, отправил на проверку;
* получил ответ от код-ревьюера: критические замечания устранены. Все отлично. Можно двигаться дальше.

**24.05.2022** (ПР 9) - приступил к выполнению практической работы по 9-му спринту

* теорию я успел пройти ранее, поэтому сразу приступаю к ПР9;
* настроил деплой проекта на github pages (сборка Вебпаком выполняется автоматически);
* добавил новые попапы для изменения аватара пользователя и для подтверждения удаления карточки;
* добавил счетчик лайков;
* приступил к изменениям в js-коде;

**25.05.2022** (ПР 9)

* переписал практически весь код - работаю над ошибками (работает не весь функционал);
* данные с сервера поступают;
* initialCards отрисовываются;
* лайки ставятся и убираются (счетчик работает);

**26.05.2022** (ПР 9)

* работаю над ошибками;
* вроде бы все устранил;
* отправил на проверку код-ревьюеру.

**27.05.2022** (ПР 9)

* получил результаты проверки код-ревьюером: 4 критических замечания и 6 рекомендаций.
* Хочу отметить, что рекомендации просто потрясающие! - столько дельных советов, что у меня, прям, азарт зашкаливает.
* приступил к исправлениям;
* все сделал, осталось реализовать одну рекомендацию по использованию универсального FormValidator;
* отправил на повторную проверку код-ревьюеру.

**28.05.2022** (ПР9)

* получил ответ от код-ревьюера: критические замечания устранены. Все отлично. Проект полностью реализован.
* на этом все. Думаю, я еще несколько раз вернусь к этому проекту, чтобы что-то подчистить и немного допилить.
